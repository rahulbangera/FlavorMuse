...
async function getBundledNames(){
  try{ const r=await fetch("/data/recipes.json",{cache:"no-store"}); if(!r.ok) throw new Error(); const arr=await r.json(); return Array.isArray(arr)?arr.map(norm):[]; }
  catch{ return []; }
}
...
export default function Dashboard(){
  ...
  function dedupeAndSort(arr){
    ...
    out.sort((a,b)=>a.localeCompare(b, undefined, { sensitivity:"base" })); return out;
  }

  async function signOut(){ localStorage.removeItem("guest"); await supabase.auth.signOut(); window.location.assign("/"); }
  ...
  return (
    <section className="container-xl py-12">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="avatar">
            {avatar ? <img referrerPolicy="no-referrer" src={avatar} alt={name} className="h-full w-full object-cover" onError={(e)=>{e.currentTarget.style.display="none"}}/> : <div className="text-sm">{name.slice(0,2).toUpperCase()}</div>}
          </div>
          <div>
      ...
      <div className="mt-6">
        <input className="field" placeholder="Search recipes…" value={q} onChange={(e)=>setQ(e.target.value)} aria-label="Search recipes" />
          ...
          {filtered.map((n)=>(
            <div key={n} className="card p-6 relative">
              <button
                ...
                onClick={(e)=>{ e.stopPropagation(); toggleFavorite({ name: n, image: "" }); const all=getFavorites(); setFavs(all); setFavSet(new Set(all.map(f=>f.name))); }}
              >
                {favSet.has(n) ? "★" : "☆"}
              </button>
      ...
      {!loading && !filtered.length && <div className="p-8 text-center text-neutral-600">No matching recipes</div>}
        ...
        {favs.slice(0,6).map((f)=>(
          <div key={f.name} className="card overflow-hidden relative">
            <button
              ...
              onClick={()=>{ toggleFavorite({ name:f.name, image:f.image||"" }); const all=getFavorites(); setFavs(all); setFavSet(new Set(all.map(x=>x.name))); }}
            >
              ★
            </button>
            <Link to={`/recipe/${f.name}`} className="block">
              <div className="aspect-[16/9] bg-neutral-100 overflow-hidden">
                {f.image ? <img src={f.image} alt={f.name} className="w-full h-full object-cover" onError={(e)=>{e.currentTarget.style.display="none"}}/> : null}
              </div>
              <div className="p-5">
